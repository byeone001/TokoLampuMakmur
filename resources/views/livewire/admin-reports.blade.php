        ...
        <table class="min-w-full divide-y divide-gray-200 border">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Kasir
                    </th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @foreach($transactions as $transaction)
                    <tr>
                        <td class="px-3 py-2 whitespace-nowrap text-sm text-gray-900">#{{ $transaction->daily_sequence }}
                        </td>
                        <td class="px-3 py-2 whitespace-nowrap text-sm text-gray-500">
                            {{ $transaction->created_at->format('d/m/Y H:i') }}
                        </td>
                        <td class="px-3 py-2 whitespace-nowrap text-sm text-gray-500">{{ $transaction->user->name ?? '-' }}
                        </td>
                ...
                <div class="p-6 overflow-y-auto max-h-[70vh]">
                    <div class="mb-4 grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <span
                                class="font-semibold">{{ $selectedTransaction->user->name ?? $selectedTransaction->user->username  }}</span>
                        </div>
                        <div class="text-right">
                            <span class="text-gray-600">Tanggal:</span>
                            <span class="font-semibold">{{ $selectedTransaction->created_at->format('d/m/Y H:i') }}</span>
                        </div>
                    </div>
                        ...
                        <tfoot class="bg-gray-50">
                            <tr>
                                ...
                                <td class="px-3 py-2 text-right font-bold text-blue-600">Rp
                                    {{ number_format($selectedTransaction->total_price, 0, ',', '.') }}
                                </td>
                            </tr>
                            <tr>
                                <td colspan="2" class="px-3 py-2 text-right text-sm">Tunai</td>
                                <td class="px-3 py-2 text-right text-sm">Rp
                                    {{ number_format($selectedTransaction->amount_paid, 0, ',', '.') }}
                                </td>
                            </tr>
                            <tr>
                                <td colspan="2" class="px-3 py-2 text-right text-sm">Kembalian</td>
                                <td class="px-3 py-2 text-right text-sm">Rp
                                    {{ number_format($selectedTransaction->change_amount, 0, ',', '.') }}
                                </td>
                            </tr>
                        </tfoot>
                    </table>
